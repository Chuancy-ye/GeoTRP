...
ui <- fluidPage(
  ...
  font-weight: 500;color: #ad1d28;")),
    ...
    selectInput(inputId ="magnesium", "magnesium", list("base" = "Mg", "other" = "add")),
   
    sliderInput("temperature", HTML("Temperature (&deg;C)"),
                min = 15, max = 300, value = 25, step = 5),
    sliderInput("concentration", HTML("concentration (Mg2+)"),
...
function1 <- function(input,temp,conc,return.ispecies = FALSE){
  ...
  if(input$addC&input$addS)
  {
    ...
    T=25
    P=1
    basis(c("MgCO3", "SO4-2", "H2O", "H+", "e-", "CO3-2"))
    basis("SO4-2", c(-6))
    basis("CO3-2", c(0))
    species(c("Mg+2"),c)
    iaq <- retrieve("Mg",c("O","H","C","S"),"aq")
    icr <- retrieve("Mg", c("O", "H","C","S"), "cr")
    species( c(iaq,icr))
    ...
    basis(c("MgCO3","H2O","H+","e-","CO2","H2S"))
    basis("CO2",c(-3))
    iaq <- retrieve("Mg",c("O","H","C"),"aq")
    icr <- retrieve("Mg", c("O", "H","C"), "cr")
    species(c("Mg+2"),c)
    species( c(iaq,icr))
    if(return.ispecies) return(species()$ispecies)
    ...
    basis(c("MgO","H2O","H+","e-","CO2","H2S"))
    basis("H2S",c(-3))
    species(c("Mg+2"),c)
    iaq <- retrieve("Mg",c("O","H","S"),"aq")
    icr <- retrieve("Mg", c("O", "H","S"), "cr")
    species( c(iaq,icr))
    if(return.ispecies) return(species()$ispecies)
    # set activities of aqueous species
    species(1:length(iaq), -6)
    ...
    #an Eh-pH diagram for Mg-bearing aqueous species
    basis(c("MgO","H2O","H+","e-"))
    species(c("Mg+2"),c)
    iaq <- retrieve("Mg",c("O","H"),"aq")
    icr <- retrieve("Mg",c("O","H"),"cr")
    species(c(iaq,icr))
    if(return.ispecies) return(species()$ispecies)
...
server <- function(input,output){
  ...
  output$text1 <- renderText({
    ...
    paste0("The system is Mg", text, ".")
  })
  output$text2 <- renderText(paste0("The temperature is ", input$temperature, " °C."
                             ,"the concentration of Mg is " , input$concentration,"."))
