...
# v2.0 (2019010.6): initial version
#Arsenic
library(shiny)
library(CHNOSZ)
ui <- fluidPage(
  ...
  font-weight: 500;color: #ad1d28;")),
    ...
    selectInput(inputId ="Arsenic", "Arsenic", list("base" = "As", "other" = "add")),
   
    sliderInput("temperature", HTML("Temperature (&deg;C)"),
                min = 15, max = 300, value = 25, step = 5),
    sliderInput("concentration", HTML("concentration (As3+)暂时无效"),
                min = -6, max = -1, value = -6, step = 1),
...
function1 <- function(input,temp,conc,return.ispecies = FALSE){
  ...
  if(input$addC&input$addS)
  {
    ...
    T=25
    P=1
    basis(c("As", "SO4-2", "H2O", "H+", "e-", "CO3-2"))
    basis("SO4-2", c(-6))
    basis("CO3-2", c(0))
    
    iaq <- retrieve("As",c("O","H","C","S"),"aq")
    icr <- retrieve("As", c("O", "H","C","S"), "cr")
    species( c(iaq,icr))
    ...
    basis(c("As","H2O","H+","e-","CO2","H2S"))
    basis("CO2",c(-3))
    iaq <- retrieve("As",c("O","H","C"),"aq")
    icr <- retrieve("As", c("O", "H","C"), "cr")
    ...
    iaq <- retrieve("As",c("O","H","S"),"aq")
    icr <- retrieve("As", c("O", "H","S"), "cr")
    species( c(iaq,icr))
    if(return.ispecies) return(species()$ispecies)
    # set activities of aqueous species
    species(1:length(iaq), -6)
    ...
    #an Eh-pH diagram for As-bearing aqueous species
    basis(c("As","H2O","H+","e-"))
    
    iaq <- retrieve("As",c("O","H"),"aq")
 ##  icr <- retrieve("As",c("O","H"),"cr")
    species(c(iaq))
    if(return.ispecies) return(species()$ispecies)
...
server <- function(input,output){
  ...
  output$text1 <- renderText({
    ...
    paste0("The system is As", text, ".")
  })
  output$text2 <- renderText(paste0("The temperature is ", input$temperature, " °C."
                             ,"the concentration of As is 10^ " , input$concentration,"."))
