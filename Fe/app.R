...
function1 <- function(input,temp,conc,return.ispecies = FALSE){
  ...
  if(input$addC&input$addS)
  {
    ...
    basis(c("Fe","H2O","SiO2","H+","e-"))
    species(c("Fe+2", "Fe+3"), c)
    iaq <- retrieve("Fe", c("O", "H","Si"), "aq")
    icr <- retrieve("Fe", c("O", "H","Si"), "cr")
    species( c(iaq,icr))
    if(return.ispecies) return(species()$ispecies)
   ...
   # icr <- retrieve("Fe", c("O", "H"), "cr")
    species(c(iaq))
    #species( c("Fe+2","Fe+3","HFeO2-","HFeO2","FeO2-"))
    if(return.ispecies) return(species()$ispecies)
    ...
    legend("bottomleft", legend = dprop, bty = "n")
    t1<-"Fe-O-H"
    t2 <- "Checked phase diagram 2019.9.10"
    mtitle(as.expression(c(t1,t2)), cex = 0.95)
    reset()
  }
...
server <- function(input,output){
  ...
  output$text1 <- renderText({
    ...
    if(input$addSi) text <-paste0("-Si",text)
   
 
    
    paste0("The system is Fe", text, ".")
  })
