...
ui <- fluidPage(
  ...
  font-weight: 500;color: #ad1d28;")),
    ...
    selectInput(inputId ="copper", "copper", list("base" = "Cu", "other" = "add")),
   
    sliderInput("temperature", HTML("Temperature (&deg;C)"),
                min = 15, max = 300, value = 25, step = 5),
    sliderInput("concentration", HTML("concentration (Cu+,Cu2+)"),
...
function1 <- function(input,temp,conc, return.ispecies = FALSE){
  ...
  if(input$addC&input$addS)
  {
    ...
    T=25
    P=1
    basis(c("CuO", "SO4-2", "H2O", "H+", "e-", "CO3-2"))
    basis("SO4-2", c(-6))
    basis("CO3-2", c(0))
    species(c("Cu+2", "Cu+2"),c)
    iaq <- retrieve("Cu",c("O","H","C","S"),"aq")
    icr <- retrieve("Cu", c("O", "H","C","S"), "cr")
    species( c(iaq,icr))
    ...
    t1<-"Cu-C-S-O-H"
    t2 <- "Checked phase diagram 2019.9.12"
    ...
    basis(c("Cu","H2O","H+","e-","CO2","H2S"))
    basis("CO2",c(-3))
    iaq <- retrieve("Cu",c("O","H","C"),"aq")
    icr <- retrieve("Cu", c("O", "H","C"), "cr")
    species(c("Cu+2", "Cu+2"), c)
    species( c(iaq,icr))
    ...
    basis("H2S",c(-3))
    species(c("Cu+2", "Cu+2"), c)
    iaq <- retrieve("Cu",c("O","H","S"),"aq")
    icr <- retrieve("Cu", c("O", "H","S"), "cr")
    species( c(iaq,icr))
    if(return.ispecies) return(species()$ispecies)
    # set activities of aqueous species
    species(1:length(iaq), -6)
    ...
    basis(c("Cu","H2O","H+","e-"))
    species(c("Cu+2", "Cu+2"),c )
  species(c("Cu","Cu2O","Cu+2","CuO","CuO2-2"))
    ##iaq <- retrieve("Cu", c("O", "H"), "aq")
   ## species(c(iaq))
    if(return.ispecies) return(species()$ispecies)
...
server <- function(input,output){
  ...
  output$text1 <- renderText({
    ...
    paste0("The system is Cu", text, ".")
  })
  output$text2 <- renderText(paste0("The temperature is ", input$temperature, " °C."
                             ,"the concentration of Cu is 10^" , input$concentration,"."))
