...
ui <- fluidPage(
  ...
  font-weight: 500;color: #ad1d28;")),
    ...
    selectInput(inputId ="Znic", "Zinc", list("base" = "Zn", "other" = "add")),
   
    sliderInput("temperature", HTML("Temperature (&deg;C)"),
                min = 15, max = 300, value = 25, step = 5),
    sliderInput("concentration", HTML("concentration (Zn2+)"),
...
function1 <- function(input,temp,conc,return.ispecies =FALSE){
  ...
  if(input$addC&input$addS)
  {
    ...
    T=25
    P=1
    basis(c("ZnO", "SO4-2", "H2O", "H+", "e-", "CO3-2"))
    basis("SO4-2", c(-6))
    basis("CO3-2", c(0))
    species(c("Zn+2"),c)
    ...
    #an Eh-pH diagram for Zn-bearing aqueous species
    basis(c("ZnO","H2O","H+","e-"))
    species(c("Zn+2"),c)
    iaq <- retrieve("Zn",c("O","H"),"aq")
 ##  icr <- retrieve("Zn",c("O","H"),"cr")
    species(c(iaq))
    if(return.ispecies)return(species()$ispecies)
...
server <- function(input,output){
  ...
  output$text1 <- renderText({
    ...
    paste0("The system is Zn", text, ".")
  })
  output$text2 <- renderText(paste0("The temperature is ", input$temperature, " °C."
                             ,"the concentration of Zn is 10^" , input$concentration,"."))
