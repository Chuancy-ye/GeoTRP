...
ui <- fluidPage(
  ...
  font-weight: 500;color: #ad1d28;")),
    ...
    selectInput(inputId ="Aluminum", "Aluminum", list("base" = "Al", "other" = "add")),
   
    sliderInput("temperature", HTML("Temperature (&deg;C)"),
                min = 15, max = 300, value = 25, step = 5),
    sliderInput("concentration", HTML("concentration (Al3+)"),
...
function1 <- function(input,temp,conc, return.ispecies = FALSE){
  ...
  if(input$addC&input$addS)
  {
    ...
    T=25
    P=1
    basis(c("Al2O3", "SO4-2", "H2O", "H+", "e-", "CO3-2"))
    basis("SO4-2", c(-6))
    basis("CO3-2", c(0))
    species(c("Al3+"),c)
    iaq <- retrieve("Al",c("O","H","C","S"),"aq")
    icr <- retrieve("Al", c("O", "H","C","S"), "cr")
    species( c(iaq,icr))
    ...
    basis(c("Al2O3","H2O","H+","e-","CO2","H2S"))
    basis("CO2",c(-3))
    iaq <- retrieve("Al",c("O","H","C"),"aq")
    icr <- retrieve("Al", c("O", "H","C"), "cr")
    species(c("Al+3"), c)
    species( c(iaq,icr))
    if(return.ispecies) return(species()$ispecies)
    ...
    basis("H2S",c(-3))
    species(c("Al+3"), c)
    iaq <- retrieve("Al",c("O","H","S"),"aq")
    icr <- retrieve("Al", c("O", "H","S"), "cr")
    species( c(iaq,icr))
    if(return.ispecies) return(species()$ispecies)
    # set activities of aqueous species
    species(1:length(iaq), -6)
    ...
    basis(c("Al2O3","H2O","H+","e-"))
    species(c("Al+3"),c)
    iaq <- retrieve("Al",c("O","H"),"aq")
    species(c(iaq))
    if(return.ispecies) return(species()$ispecies)
    ...
    legend("bottomleft", legend = dprop, bty = "n")
    t1<-"Al-O-H"
    t2 <- "Unchecked phase diagram 2019.9.29"
    mtitle(as.expression(c(t1,t2)), cex = 0.95)
    reset()
}
...
server <- function(input,output){
  ...
  output$text1 <- renderText({
    ...
    paste0("The system is Al", text, ".")
  })
  output$text2 <- renderText(paste0("The temperature is ", input$temperature, " °C."
                             ,"the concentration of Al is " , input$concentration,"."))
