...
ui <- fluidPage(
  ...
  font-weight: 500;color: #ad1d28;")),
    ...
    selectInput(inputId ="iron", "Iron", list("base" = "Fe", "unsetP" = "Fe(unset P)")),
    ...
    checkboxInput("unsetP", "Fe(unset P)，T > 100℃", FALSE),
    hr(),
...
function1 <- function(input,temp){
  ...
  if(input$addC&input$addS)
  {
    ...
    m <- mosaic(bases,pH = c(0, 14,res), Eh = c(-0.8, 1.2,res),T=T)
    ...
    dprop <- describe.property(c("T"), c(T))
    #add legend and title
    ...
    species( c(iaq))
    a <- affinity(pH = c(0, 14,res), Eh = c(-0.8, 1.2,res),T=T)
...
server <- function(input,output){
  ...
  output$text1 <- renderText({
   ...
   if(input$unsetP) {text <- paste0("Fe(unsetP)", text)}else{
     text <- paste0("Fe", text)
   }
  ...
  output$chnosz.diagram <- renderPlot({
    ...
    args <- list(input = input,temp =input$temperature)
