...
ui <- fluidPage(
  ...
  font-weight: 500;color: #ad1d28;")),
    ...
    selectInput(inputId ="manganese", "manganese", list("base" = "Mn", "other" = "add")),
   
    sliderInput("temperature", HTML("Temperature (&deg;C)"),
                min = 15, max = 300, value = 25, step = 5),
    sliderInput("concentration", HTML("concentration (Mn4+,Mn2+,Mn3+)"),
...
function1 <- function(input,temp,conc,return.ispecies = FALSE){
  ...
  if(input$addC&input$addS)
  {
    ...
    T=25
    P=1
    basis(c("MnO", "SO4-2", "H2O", "H+", "e-", "CO3-2"))
    basis("SO4-2", c(-6))
    basis("CO3-2", c(0))
    species(c("Mn+2", "Mn+3"),c)
    if(return.ispecies)return(species()$ispecies)
    iaq <- retrieve("Mn",c("O","H","C","S"),"aq")
    icr <- retrieve("Mn", c("O", "H","C","S"), "cr")
    species( c(iaq,icr))
    ...
    basis(c("MnO","H2O","H+","e-","CO2","H2S"))
    basis("CO2",c(-3))
    iaq <- retrieve("Mn",c("O","H","C"),"aq")
    icr <- retrieve("Mn", c("O", "H","C"), "cr")
    species(c("Mn+2", "Mn+3"),c)
    species( c(iaq,icr))
    if(return.ispecies)return(species()$ispecies)
    ...
    basis("H2S",c(-3))
    species(c("Mn+2", "Mn+3"),c)
    iaq <- retrieve("Mn",c("O","H","S"),"aq")
    icr <- retrieve("Mn", c("O", "H","S"), "cr")
    species( c(iaq,icr))
    if(return.ispecies)return(species()$ispecies)
    # set activities of aqueous species
    species(1:length(iaq), -6)
...
server <- function(input,output){
  ...
  output$text1 <- renderText({
    ...
    paste0("The system is Mn", text, ".")
  })
  output$text2 <- renderText(paste0("The temperature is ", input$temperature, " °C."
                             ,"the concentration of Mn is 10^ " , input$concentration,"."))
